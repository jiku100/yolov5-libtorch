...
Detector::Run(const cv::Mat& img, float conf_threshold, float iou_threshold) {
    ...
    std::vector<float> pad_info = LetterboxImage(img_input, img_input, cv::Size(1920, 1088));
    ...
    std::cout << "pre-process takes : " << duration.count() << " us" << std::endl;
    ...
    // It should be known that it takes longer time at first time
    std::cout << "inference takes : " << duration.count() << " us" << std::endl;
    ...
    duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
    // It should be known that it takes longer time at first time
    std::cout << "post-process takes : " << duration.count() << " us" << std::endl;
